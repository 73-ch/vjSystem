...
vec3 AND(float a){float r=(a - 0.375)*PI_2;float p=smoothstep(.85,1.,a);float q=step(-.125,-a);float x=sin(r*2.)*.3*(1.+q*.35)*(1.+p*.35);float y=cos(r)*(1.+q*.5)/(1.+p*pow(sin(r-PI),2.)*2.)*.5;return vec3(x,y,.0);}
...
vec3 createTsukuba(wordinput){float n=mod(id*7.,v_num);float l=v_num/7.;if(n<=l)return T(n/l)*si+vec3(-(sp)*4.5,.0,.0);if(n<=l*2.)return S((n-l)/l)*si+vec3(-(sp)*3.,.0,.0);if(n<=l*3.)return U((n-l*2.)/l)*si+vec3(-sp*1.75,.0,.0);if(n<=l*4.)return K((n-l*3.)/l)*si+vec3(-sp*.25,.0,.0);if(n<=l*5.)return U((n-l*4.)/l)*si+vec3((sp)*1.25,.0,.0);if(n<=l*6.)return B((n-l*5.)/l)*si+vec3((sp)*2.8,.0,.0);return A((n-l*6.)/l)*si+vec3((sp)*4.5,.0,.0);}
...
vec3 createProject(wordinput) {float n=mod(id*7.,v_num);
  float l=v_num/7.;
  if(n<=l)return P(n/l)*si+vec3(-(sp)*4.5,.0,.0);
  if(n<=l*2.)return R((n-l)/l)*si+vec3(-(sp)*3.25,.0,.0);
  if(n<=l*3.)return O((n-l*2.)/l)*si+vec3(-sp*1.75,.0,.0);
  if(n<=l*4.)return J((n-l*3.)/l)*si+vec3(-sp*.25,.0,.0);
  if(n<=l*5.)return E((n-l*4.)/l)*si+vec3((sp)*1.25,.0,.0);
  if(n<=l*6.)return C((n-l*5.)/l)*si+vec3((sp)*3.,.0,.0);
  return T((n-l*6.)/l)*si+vec3((sp)*4.5,.0,.0);


}

vec3 createMast(wordinput) {float n=mod(id*4.,v_num);float l=v_num/4.;if(n<=l)return M(n/l)*si+vec3(-(sp)*3.,.0,.0);if(n<=l*2.)return A((n-l)/l)*si+vec3(-(sp),.0,.0);if(n<=l*3.)return S((n-l*2.)/l)*si+vec3(sp,.0,.0);return T((n-l*3.)/l)*si+vec3((sp)*2.75,.0,.0);}
vec3 createMad(wordinput) {float n=mod(id*3.,v_num);float l=v_num/3.;if(n<=l)return M(n/l)*si+vec3(-(sp)*2.,.0,.0);if(n<=l*2.)return A((n-l)/l)*si+vec3(0.,.0,.0);return D((n-l*6.)/l)*si+vec3((sp)*2.0,.0,.0);}
vec3 createAandD(wordinput) {float n=mod(id*3.,v_num);float l=v_num/3.;if(n<=l)return A(n/l)*si+vec3(-(sp)*2.,.0,.0);if(n<=l*2.)return AND((n-l)/l)*si+vec3(0.,.0,.0);return D((n-l*6.)/l)*si+vec3((sp)*2.0,.0,.0);}
