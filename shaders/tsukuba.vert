...
vec3 createProject(wordinput) {float n=mod(id*7.,v_num);float l=v_num/7.;if(n<=l)return P(n/l)*si+vec3(-(sp)*4.5,.0,.0);if(n<=l*2.)return R((n-l)/l)*si+vec3(-(sp)*3.25,.0,.0);if(n<=l*3.)return O((n-l*2.)/l)*si+vec3(-sp*1.75,.0,.0);if(n<=l*4.)return J((n-l*3.)/l)*si+vec3(-sp*.25,.0,.0);if(n<=l*5.)return E((n-l*4.)/l)*si+vec3((sp)*1.25,.0,.0);if(n<=l*6.)return C((n-l*5.)/l)*si+vec3((sp)*3.,.0,.0);return T((n-l*6.)/l)*si+vec3((sp)*4.5,.0,.0);}
