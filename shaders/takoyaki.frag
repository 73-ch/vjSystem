...
void main() {
    ...
    // camera
    vec3 camera_position = vec3(4. * cos(time),3.,4. * sin(time));
    vec3 camera_dir = normalize(-camera_position);
    // vec3 camera_position = vec3(0.,2.,5.0);
    // vec3 camera_dir = vec3(0.,-.5,-1.0);

    vec3 camera_up = vec3(0.,-1.,0.);
    vec3 camera_side = cross(camera_dir, camera_up);
    float target_depth = 1.0;
    
    vec3 ray = normalize(camera_side * p.x + camera_up * p.y + camera_dir * target_depth);
